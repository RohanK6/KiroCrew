...
# separate tokens and misses the assembled `~/.ssh/id_rsa`. Match a
# ``NAME=VALUE`` shell assignment at the start of the command or after a
# ``;``/``&&``/``||`` separator. VALUE runs to whitespace/;/&/| (an unquoted
# shell word).
_CRON_LOCAL_ASSIGN_RE = re.compile(
    r"(?:^|[;&|]\s*)"          # start-of-command or after a shell separator
...
def _substitute_local_assignments(command: str) -> str:
    ...
    # Replace ``$NAME`` and ``${NAME}`` for every assignment. Longest-name first
    # so ``$AB`` never accidentally matches ``$A``. This is a static rescan
    # helper, not a shell — no globbing, no arithmetic; unresolved names remain
    # literal, which is what we want (the follow-up cred-path check can't false-
    # negative on a literal ``$UNKNOWN`` that resolves to something at runtime).
    for name in sorted(assigns, key=len, reverse=True):
        value = assigns[name]
        command = re.sub(r"\$\{" + re.escape(name) + r"\}", value, command)
        command = re.sub(r"\$" + re.escape(name) + r"(?![A-Za-z0-9_])", value, command)
    return command
