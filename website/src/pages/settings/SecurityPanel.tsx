...
import { ShieldCheck, ShieldAlert, Lock, Eye, EyeOff, FileWarning, Terminal, Globe, Fingerprint, KeyRound, ScanLine, Layers, AlertTriangle, CheckCircle2, Circle, Clock, ExternalLink, ChevronRight, ChevronDown, Plus, Trash2, Gavel, Building2, Gauge, ToggleRight, MessageSquare, ListChecks, Package } from 'lucide-react'
import { useAppSelector } from '../../store'
import { Badge, Btn, Input, Toggle, Checkbox } from '../../components/ui'
import { SettingsSection, SettingsCard, SettingsToggle } from '../../components/settings'
import Modal from '../../components/Modal'
import InfoTip from '../../components/InfoTip'
import { api, ApiError, type DeniedCommandsData, type DeniedCommandRule, type DeniedUserRule, type GovernancePolicyData, type GovernanceScope, type GovernanceScopeDetail, type SecurityPostureData, type TrustedAppsData } from '../../api/client'
 ...
 * since `config.local.json` is user-owned and never written by KiroCrew), and
...
export function SecurityPanel() {
  ...
  // Third-party app trust. Separate query from denied-commands: a different
  // endpoint, a different snapshot, and it must keep rendering when either of
  // the two fails.
  ...
  // A trust mutation can now FAIL in ways the operator has to see rather than
  // infer from a toggle springing back: the setting may be owned by
  // config.local.json (so writing config.json would change nothing), or the
  // blanket-off sweep may have left apps running. Both arrive as a 409 with a
  // `code`, and both mean "the thing you asked for did not happen" — silence here
  // would reproduce, in the UI, exactly the false-success the backend fixed.
  ...
  const runConfirm = () => {
    ...
    else if (confirm.kind === 'trust-all') setTrustAll.mutate(true)
    else if (confirm.kind === 'revoke-app') untrust.mutate(confirm.name)
  ...
  const confirmBody = !confirm ? '' : confirm.kind === 'trust-all'
    ...
    : confirm.kind === 'revoke-app'
      ? i18nT('pages.settings.securityPanel.trustedApps.revoke_confirm_body', { name: confirm.name })
      : confirm.kind === 'disable-all'
        ? i18nT('pages.settings.securityPanel.disabling_all_built_in_denies_removes_kirocrew_s')
        : i18nT('pages.settings.securityPanel.disabling_rule_weakens_protection', {
          description: confirm.description,
        })
  // The ack sentence differs for trust-all: the deny copy names KiroCrew's own
  // command gate, which is not what a blanket app-trust grant weakens.
  const confirmAck = confirm?.kind === 'trust-all'
    ? i18nT('pages.settings.securityPanel.trustedApps.allow_all_confirm_ack')
    : i18nT('pages.settings.securityPanel.i_understand_this_weakens_kirocrew_s_protection')
  ...
  const needsAck = confirm?.kind !== 'revoke-app'
  ...
  return (
    <>
      ...
      {/* ── Trusted third-party apps ──
          Third-party app code does not run unless it is trusted. Two levers:
          the per-app grants listed here, and the blanket allow-all flag. The
          allow-all row is a `SettingsToggle` rather than a hand-rolled row so it
          carries `data-setting-label` (the deep-link target the App Store links
          to) and is picked up by scripts/gen-settings-registry.mjs. */}
      <SettingsSection title={i18nT('pages.settings.securityPanel.trustedApps.label')}>
        <SettingsCard>
          ...
          {/* UNKNOWN is not OFF. If the snapshot has not resolved, the persisted
              ...
              disabled switch, since it resolves on its own. (Semantics carried
              over from the standalone card #1414 added, which this replaces.) */}
          {taUnavailable ? (
      ...
      {/* ── Confirm modal (disable a built-in rule / disable all / trust every app) ── */}
      <Modal
        open={confirm !== null}
      ...
      >
        ...
        {needsAck && (
          <label className="flex items-center gap-2.5 mt-4 cursor-pointer">
            ...
            <span className="text-[13px] text-text">{confirmAck}</span>
          </label>
        )}
      </Modal>
    </>
