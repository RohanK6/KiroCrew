// AppleTranscribe.swift — on-device speech-to-text helper for KiroCrew's `apple` STT provider.
//
// WHY A SEPARATE BINARY: Apple's SpeechAnalyzer / SpeechTranscriber (macOS 26+) is a
// Swift-only framework. The KiroCrew gateway is Python, so the only way to reach it is
