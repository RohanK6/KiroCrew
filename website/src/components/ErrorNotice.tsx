...
export default function ErrorNotice({
  ...
  const resolved = report ?? findReport(message)
  ...
  if (variant === 'inline') {
    return (
      <span role="alert" className={`inline-flex items-center gap-1.5 text-[12px] text-danger ${className}`}>
        ...
        {askAgent && <AskAgentButton report={resolved} message={message} />}
        {onDismiss && (
          <button
            type="button"
  ...
  return (
    ...
    >
      ...
      {askAgent && <AskAgentButton report={resolved} message={message} className="mt-[1px]" />}
      {onDismiss && (
        <button
          type="button"
          className="shrink-0 bg-transparent border-none p-0 cursor-pointer text-danger/70 hover:text-danger transition-colors"
