...
import { lucideFolderIcon } from '../components/folderIconCatalog'
import { FOLDER_BACK_FILL, FOLDER_OUTLINE, FOLDER_OUTLINE_PX, folderBackMasks } from '../components/folderGlyphMasks'
...
function FolderGlyph({ icon, size = 20, open = false, className = 'shrink-0', testId }: { icon?: string; size?: number; open?: boolean; className?: string; testId?: string }) {
  const h = Math.round(size * 0.875)
  const iconPx = Math.max(9, Math.round(size * 0.5))
  // Badge: a curated lucide icon — a stroke SVG that tints from the theme via
  // currentColor and deforms natively under the open tilt. Unknown values
  // (legacy emoji awaiting migration) resolve to null and render no badge.
  const Badge = lucideFolderIcon(icon)
  const ease = 'transform .22s ease'
  return (
    <span data-testid={testId} aria-hidden className={`relative inline-flex ${className}`} style={{ width: size, height: h }}>
      ...
      {(() => {
        ...
        return (
          <span className="absolute inset-0" style={{ transform: open ? 'skewX(8deg)' : 'none', transformOrigin: '50% 100%', transition: ease }}>
             ...
             *  inside-aligned at exactly FOLDER_OUTLINE_PX and hugs the
             *  silhouette. */}
            <span className="absolute inset-0" style={{ background: FOLDER_BACK_FILL, WebkitMaskImage: masks.fill, maskImage: masks.fill, WebkitMaskRepeat: 'no-repeat', maskRepeat: 'no-repeat' }}>
              <span className="absolute inset-0" style={{ background: FOLDER_OUTLINE, WebkitMaskImage: masks.line, maskImage: masks.line, WebkitMaskRepeat: 'no-repeat', maskRepeat: 'no-repeat' }} />
            </span>
          </span>
        )
      })()}
      {/* front panel — outlined rect covering the body up to just under the
       *  tab; carries the badge. Open tilts it right (−11°) with a squash, so
       *  the duo-tilt splits the motion and the badge shears with the face
       *  like paint on it. */}
      <span className="absolute flex items-center justify-center" style={{ left: '2%', right: '2%', top: '22%', bottom: '2%', borderRadius: '10% / 14%', background: 'color-mix(in srgb, var(--accent) 30%, var(--bg-elevated))', border: `${FOLDER_OUTLINE_PX}px solid ${FOLDER_OUTLINE}`, transform: open ? 'skewX(-11deg) scaleY(.9)' : 'none', transformOrigin: '50% 100%', transition: ease }}>
        {Badge && <Badge size={iconPx} strokeWidth={2.25} className="pointer-events-none" style={{ color: 'color-mix(in srgb, var(--text-strong) 80%, var(--accent))' }} />}
      </span>
    </span>
  )
}
  ...
  const renderColumnFolder = (folder: ChatFolder, columnId: string, colSlotKeys: Set<string>, dragHandleProps?: React.HTMLAttributes<HTMLElement>, forceCollapsed?: boolean): React.ReactNode => {
    ...
    return (
      ...
      >
        ...
        >
          <FolderGlyph icon={folder.icon} size={15} open={!folder.collapsed} className="shrink-0" />
    ...
    return (
      <div key={`folder-header-${folder.id}`}
        ...
        {editingId === folder.id && editScope === 'list' ? (
          <>
            <FolderGlyph icon={folder.icon} size={19} open={!folder.collapsed} />
          ...
          <>
            ...
            <button type="button"
              ...
              aria-label={folder.collapsed ? i18nT('pages.chatSidebar.expand_folder_name', { name: folder.name }) : i18nT('pages.chatSidebar.collapse_folder_name', { name: folder.name })}
              onClick={() => toggleCollapse(folder.id)}>
              <FolderGlyph icon={folder.icon} size={19} open={!folder.collapsed} testId={`folder-collapse-${folder.id}`} />
              {/* Double-click rename is a mouse-only power shortcut; the accessible
               *  path is the ⋯-menu Rename item, so scope-disable the interaction rule. */}
              {/* eslint-disable-next-line jsx-a11y/no-static-element-interactions */}
