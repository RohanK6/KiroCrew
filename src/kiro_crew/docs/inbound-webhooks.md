...
This is the inbound counterpart to cron jobs: cron fires on a schedule KiroCrew
owns, a webhook fires when something outside KiroCrew decides it is time.
...
The webhook token is the **only** thing this endpoint checks. There is no second
network gate in front of it.
...
- A legacy single token set as `hooks.webhook_token` in `config.json` continues
  ...
  the key from the config file — KiroCrew does not rewrite your config for you.
