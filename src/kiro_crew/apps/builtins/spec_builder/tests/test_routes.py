...
def test_halt_execution_writes_the_sentinel_off_the_loop():
    ...
    assert "asyncio.to_thread(_write_stop_sentinel" in src
...
def test_no_handler_reads_the_index_on_the_event_loop():
    ...
    # only ever runs via asyncio.to_thread. The BLOCKING assertion below ties
    # this allowance to that contract, so the set cannot be widened to admit a
    ...
    for off_loop in ("_load_index_with_discovery", "_prepare_handoff"):
