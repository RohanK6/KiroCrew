...
`/api/ws` and `/api/status` are implicitly allowed for app tokens
(`_APP_TOKEN_IMPLICIT_ALLOW`) rather than requiring every app to declare the transport;
that grant is recorded in the Security Event Log.
