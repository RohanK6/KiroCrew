...
def _prepare_handoff(
    ...
    lock BEFORE the sentinel is cleared, and a mismatch refuses. Arming is
    destructive -- it removes the STOP that a Pause wrote -- so it must not
    happen for a spec this request no longer refers to: a stale same-name,
    same-path execute would otherwise clear a REPLACEMENT's stop and let the
    persisted loop resume after a restart. Gating the act itself is what covers a
    request carrying no client claim, which no claim comparison can refuse.
    """
...
async def _halt_execution(
    ...
    *,
    ...
    # the duration of a Stop click.
    if not await asyncio.to_thread(_write_stop_sentinel, spec_dir):
