"""Select-to-Edit backend for KiroCrew.
...
Bound to localhost; KiroCrew proxies `/apps/design-tweak/api/*` to
...
_KIROCREW_INTERNAL_DIRS: tuple[str, ...] = tuple(
    ...
    for p in (
        ...
        str(DATA_DIR),  # a relocated data home (KIROCREW_APP_DATA_DIR)
    )
)


def _is_kirocrew_internal(target: Path) -> bool:
    """Whether *target* resolves inside one of KiroCrew's own directory trees.
...
_PROJECT_SECRET_NAMES: frozenset[str] = frozenset(
    {
        ...
        ".git-credentials",
        ".htpasswd",
        # KiroCrew's per-app proxy-auth HMAC credential. Also covered by
...
# Whole directories, matched on any path component.
_PROJECT_SECRET_DIRS: frozenset[str] = frozenset(
    {".git", ".hg", ".svn", ".ssh", ".aws", ".gnupg", ".gpg", ".azure", ".kube"}
)
...
def _static_response(
    ...
    # KiroCrew's own data home, but NOT a credential file sitting inside the
    ...
    if _is_project_secret(root, target):
        return 403, "text/plain", b"forbidden"
    # KiroCrew's own trees are refused outright, whatever the project root is.
...
class Handler(BaseHTTPRequestHandler):
    server_version = "KiroCrew-SelectToEdit/" + VERSION
