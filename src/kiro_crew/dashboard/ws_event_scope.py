...
# the same pair within the window are silently counted but not emitted.
# ---------------------------------------------------------------------------

_SEL_DEDUP_WINDOW_SECS = 300.0  # 5 minutes
_sel_last_audit: dict[tuple[str, str, str], float] = {}
...
def _audit_deny(app: str, event_type: str, reason: str) -> None:
    ...
    now = time.monotonic()
    last = _sel_last_audit.get(key)
    if last is not None and (now - last) < _SEL_DEDUP_WINDOW_SECS:
        return
    _sel_last_audit[key] = now
    try:
