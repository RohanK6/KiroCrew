...
/** Curated lucide icons pickable as chat-folder badges.
 *
 *  Stored in the folder's `icon` field as `lucide:<name>` (kebab-case lucide
 *  name); plain single-emoji values remain supported as legacy data and render
 *  through the emoji path in FolderGlyph. Lucide badges are stroke SVGs, so
 *  they inherit theme tinting via `currentColor` and deform natively under the
 *  open-folder tilt (no canvas rasterization needed, unlike bitmap emoji).
 *
 *  KEEP IN SYNC with `_FOLDER_LUCIDE_NAMES` in
 *  src/kiro_crew/dashboard/chat_folders.py — the backend allowlist that
 *  validates PATCHed icons and constrains the auto-generator. */
