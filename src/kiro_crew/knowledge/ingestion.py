...
class IngestionPipeline:
    ...
    async def ingest_file(self, path: str, on_progress=None, original_name: str = "", namespace: str = "default", source_id: str = "", old_item_ids: list[str] | None = None) -> str | None:
        ...
        # Separate copy for messages and audit records. display_name is
        # caller-supplied (an upload's filename, a folder file's name, a document
        # title) and a name is free-form enough to carry a credential, so every
        # sink that persists or logs it uses the redacted form. The stored
        # display_name is untouched -- it is the document's title. No ``or``
        # fallback: _redact returns its input unchanged when falsy, so a fallback
        # could only ever re-yield the same empty string while handing static
        # analysis a genuine unredacted edge into these sinks.
        ...
        if limit_mb > 0 and file_size > limit_mb * _MB:
            ...
            # lgtm[py/clear-text-logging-sensitive-data] -- justified by the
            # BOUNDARY, not by which caller supplied the name: log_name is derived
            # once, immediately after display_name, and every message and audit sink
            # in this function uses it (the sensitive-path event and its
            # PermissionError, this warning, and the oversized SEL event). No sink
            # reads the raw value, so the property holds for all seven ingest paths
            # and for any added later. The query flags it because it does not model
            # _redact as a sanitiser.
            logger.warning(msg)
            sel().log_tool_invocation(
                session_key="ingestion", agent="knowledge-ingest",
                tool_name="knowledge.ingest_denied", outcome="denied",
