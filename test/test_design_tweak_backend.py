...
class TestDashboardOriginRouteIsGone:
    ...
    def test_route_reports_gone_and_serves_no_bytes(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(
            server, "_CFG", {"projects": [{"id": "abc123", "path": str(tmp_path)}]}
        )
        ...
        for fn in sites:
            for line in inspect.getsource(fn).splitlines():
                ...
                if not stripped.startswith('"injected":'):
                    continue
                checked += 1
                assert "bool(framed)" in stripped, (
...
class TestProjectSecretsAreNeverServed:
    ...
    `is_sensitive_path` is HOME-relative — it covers `~/.aws` and KiroCrew's data
...
class TestKiroCrewInternalTreesAreNeverServed:
    """Registering `~` must not expose KiroCrew's OWN secrets.
...
def _static_response_at(root, rel):
    ...
    return server._static_response(
        str(root), rel, "/p1/", script=server._OVERLAY_PATH
    )
...
def static_preview(tmp_path, monkeypatch):
    ...
    def get(path, headers=None, method="GET"):
        req = urllib.request.Request(
            base.rstrip("/") + path, headers=headers or {}, method=method
        )
        try:
...
@pytest.fixture()
def queued_request():
    """One real request file in the live QUEUE_DIR, removed afterwards.

    `_write_request` re-asserts containment under the data dir, so the file has
    to live in the real queue rather than a tmp_path.
    """
...
class TestPersistedDevUrlIsFrontedWithProxy:
    ...
    def test_no_persisted_url_still_gets_the_static_preview(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(server, "_DEV_PROCS", {})
        monkeypatch.setattr(server, "_static_preview_url", lambda pid: f"http://127.0.0.1:7777/{pid}/")
