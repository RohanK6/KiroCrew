...
def resolve_origin_url(config: dict) -> str:
    ...
    ARCC's guidance on untrusted URL destinations asks for exactly this — allowlist the
...
def checkout_branch(clone: Path, branch: str, *, timeout_s: int = 120) -> tuple[bool, str]:
    ...
    def _run(*args: str, tmo: int = timeout_s) -> subprocess.CompletedProcess:
        # Harden every host-side git over this clone with `-c core.hooksPath=<devnull> -c
        # core.fsmonitor=false`: `checkout -B` below runs `post-checkout` hooks and `git`
        # consults `core.fsmonitor`, and this clone may already hold a tree a prior agent pass
        # edited — a repo-planted hook/fsmonitor program would execute host-side. Same
        # D-120 hook-hardening class as the other git helpers. Raised by the Opus 5 review.
        return subprocess.run(
            ["git", "-C", str(clone), "-c", f"core.hooksPath={os.devnull}", "-c",
