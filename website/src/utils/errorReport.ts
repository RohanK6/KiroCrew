...
/**
 * Trim + cap free-form detail. Returns undefined for nothing worth keeping.
 *
 * Caps BEFORE scrubbing: the tail past `MAX_DETAIL` is discarded either way, so
 * scrubbing a multi-megabyte body first is pure waste. Truncating first cannot
 * expose a credential — the key and the head of its value sit on the same side of
 * the cut, so the pattern still matches and redacts; a cut landing after the key
 * leaves no value to leak.
 */
function normalizeDetail(detail: string | undefined): string | undefined {
  if (!detail) return undefined
  const trimmed = detail.trim()
  if (!trimmed) return undefined
  const capped = trimmed.length > MAX_DETAIL
    ? trimmed.slice(0, MAX_DETAIL) + TRUNCATION_SUFFIX
    : trimmed
...
export function recordError(input: {
  ...
  const report: ErrorReport = {
    ...
    message: redactSecrets(
      input.message.length > MAX_MESSAGE ? input.message.slice(0, MAX_MESSAGE) + TRUNCATION_SUFFIX : input.message,
    ),
    status: input.status,
    code: input.code,
