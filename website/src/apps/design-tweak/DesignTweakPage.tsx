...
// Two-panel layout inside KiroCrew's content area: resizable left rail + preview.
...
export default function DesignTweak() {
    ...
    try {
      ...
      try {
        if (key) {
          ...
          // No folder known — nothing to key a session on, so hand it to the
          // chat composer instead of dropping it.
          dispatch(setPendingInput(msg))
          navigate(chatRoute())
          setStatus(i18nT('apps.designTweak.status.sent_request_to_agent', {
            number: snap.number ?? req.number,
          }))
        }
      } catch (dispatchErr) {
        // RETRY PATH: the request is sealed but the agent never got it. Rather
        // than leave the work stranded, put the exact prompt in the chat composer
        // so the user can send it themselves — nothing is lost, and they are told
        // plainly that this step did not complete on its own.
        dispatch(setPendingInput(msg))
        navigate(chatRoute())
        setStatus(i18nT('apps.designTweak.status.dispatch_failed_composer', {
  ...
  }, [refresh, projects, previewId, ensureSlot, dispatch, navigate])
