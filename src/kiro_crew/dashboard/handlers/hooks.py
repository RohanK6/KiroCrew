...
# are individually guarded (the kill-switch read, the last-used stamp, the run
# record), so nothing here escapes as a 500.
...
async def api_webhook_test(request: web.Request) -> web.Response:
    ...
    if not message:
        message = (
            "This is a KiroCrew webhook test request. Reply with a one-line "
