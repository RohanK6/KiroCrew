...
class CrewStore:
    ...
    def _save(self, name: str, data: list[dict[str, Any]]) -> None:
        ...
        guarded by a lock makes newest-wins deterministic even if the
        executor runs writes out of order; the lock also serializes the
        actual write+replace. Sync callers (tests, reconcile at boot)
        write inline.
        """
