...
| `secrets` | Managed integration credentials plus other encrypted vault entries; a Custom secret the owner authorizes for an exact https origin can also be used by the agent to call that API from a private member conversation whose call the runtime routes through the host (which injects an audience-bound proof), mediated host-side so the plaintext never enters the sandbox. The in-sandbox `kirocrew-secrets` MCP server is caller-aware, so the host endpoint authenticates each request with an audience-bound member-session proof (a proof minted for another server, or an unidentified caller, is refused). That proof is only minted for a private (V2) member session and must be minted host-side (where the signing key is readable); the endpoint requires it and returns 403 fail-closed for any call that arrives without a valid audience-bound proof — a default Global (V1) chat, or any path that cannot present a host-minted proof, is refused. Whether a given member-session tool call can present one depends on the runtime's MCP routing (a gateway-proxied call carries the host-injected proof); this row documents the enforcement, not a guarantee that every member-chat routing is supported. This is a deliberate scoping of a credential-bearing egress to an authenticated session. The mediated result is, by default, a fixed CONSTANT that reveals nothing about the upstream response — no body, headers, status, content-type, or length (a sentinel status of 0 and a fixed message) — so a credential the origin reflects through any transform (body split, header split, or a status/length covert channel) has nothing to ride out on; the owner opts a specific origin into receiving the real response (body + headers + status) with `secrets authorize --return-body` | `SecretsPanel.tsx` | `handlers/secrets.py`, `handlers/sessions.py` (`api_mediated_secret_request` — host-side vault-read + SSRF-checked dispatch, gated on the audience-bound proof, for the in-sandbox `kirocrew-secrets` MCP tool) | `GET,POST /api/secrets`, `DELETE /api/secrets/{name}`, `POST /api/mediated-secret-request` |
