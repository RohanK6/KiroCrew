...
def main():
    ...
    # (more than 5 commits for a single-commit PR workflow), warn loudly.  This
    # catches worktrees that were branched from a local trunk carrying unshipped
    # integration commits (root cause of the 2026-07-31 clobber).  The threshold
    # is generous — a normal prepare-pr squashes to 1 commit; 5 allows for
    # multi-commit profiles or a small rebase stack.
    if ahead != "?" and int(ahead) > 5:
        print(
