...
describe('redaction', () => {
  ...
  it('still redacts a credential that sits before the detail cap', () => {
    // Capping happens BEFORE scrubbing, so pin that the order did not open a hole:
    // a key and the head of its value on the same side of the cut still match.
    const r = recordError({
      source: 'api',
      message: 'boom',
  ...
  it('redacts URL userinfo, the shape a git remote error echoes back', () => {
    // Assembled from parts, not written as a literal URI: Semgrep's
    // detected-username-and-password-in-uri secrets rule flags the literal form,
    // and a fake credential in a redaction test is exactly the false positive
    // that rule cannot distinguish. Same input, no suppression comment needed.
